...
def vert2csv(vertfile: Union[str, Path], outfile: Union[str, Path], skiplines: list=[]):
    ...
    """
    with open(outfile, "w", encoding="utf-8") as _outfile:
        # Added errors="ignore" to bypass non-utf-8 bytes generated by MSMS
        with open(vertfile, "r", encoding="utf-8", errors="ignore") as _readfile:
            for i, line in enumerate(_readfile):
                if i in skiplines:
                    continue
                parts = line.split()
                # Added length check to avoid IndexErrors on malformed lines
                if len(parts) >= 3:
                    x, y, z = [ _.strip() for _ in parts[:3] ]
